...
RESULT ec_staticinit_configured_curve( Integer Q, EC_vars_p ec, BYTE m )
{
    ...
    printf( "\nSECP256R1_DOMAIN_PARAMS_X_LEN[%d]  SECP256R1_G[%d] res[%d]"
            " X_BUF sz[%d]\n",
    ...
    if( m == EC_INIT_MODE_G )
    {
        fill( ec->X->buf, ec->X->size, CURVE_X(G), CURVE_XLEN(G) );
        setOffset(ec->X);
        fill( ec->X1->buf, ec->X1->size, CURVE_X(G), CURVE_XLEN(G) );
        setOffset(ec->X1);
        fill( ec->Y->buf, ec->Y->size, CURVE_Y(G), CURVE_YLEN(G) );
        setOffset(ec->Y);
        fill( ec->Y1->buf, ec->Y1->size, CURVE_Y(G), CURVE_YLEN(G) );
        setOffset(ec->Y1);
        fill( ec->a->buf, ec->a->size, CURVE(a), CURVE_PARMSZ(a) );
        setOffset(ec->a);
        ...
        fill( ec->p->buf, ec->a->size, CURVE(n), CURVE_PARMSZ(n) );
        setOffset(ec->p);
        /* barrett reductions modulo n consistent with above */
        fill( ec->Mu->buf, ec->Mu->size, CURVE(m), CURVE_PARMSZ(m) );
        setOffset(ec->Mu);
        _res_ = OK;
    } else
    {
...
*/
RESULT ec_init_vars( BYTE curve, Integer Q, EC_vars_p ec, BYTE m )
{
    if( curve == CURVE(ID) )
    {
...
RESULT ec_set_mod_param( BYTE curve, EC_vars_p ec, BYTE mode )
{
    ...
    {
        fill( ec->Mu->buf, ec->Mu->size, CURVE(u), CURVE_PARMSZ(u) );
        setOffset( ec->Mu );
        fill( ec->p->buf, ec->p->size, CURVE(p), CURVE_PARMSZ(p) );
        setOffset( ec->p );
    } else
    {
        fill( ec->Mu->buf, ec->Mu->size, CURVE(m), CURVE_PARMSZ(m) );
        setOffset( ec->Mu );
        fill( ec->p->buf, ec->p->size, CURVE(n), CURVE_PARMSZ(n) );
        setOffset( ec->p );
    }
    return OK;
}
