...
/*!
  ...
  \see: modinv_ since this is deprecated
*/
RESULT modinv_1( Integer A, Integer P, Integer  R,
...
/*
 * The below would have been OK if it worked
 * Unfortunately, say if r == 1, X + P can be positive
 * or negative. If negative, we need to add another for
 * loop to negate the result which defeats the idea of doing
 * things in a single pass.
 */
RESULT modinv_calc_x( Integer X, Integer P, BYTE r )
{
    SSHORT c = 0;
    SSHORT i = r;
    SWORD a = 0;
    SBYTE neg = (X->flags & NEGATIVE)? -1:1;
    SBYTE pneg = 1;
    BYTE shft = 0;
    SSHORT j = 0;
    SSHORT p = 0;
    //SHORT  m = 0;
    /* Save bit '1' if we did a shift, upto a max of 8 */
    BYTE save = 0;
    if( byte_ctz( X->buf[X->size -1] ) >= r )
    {
        rshift_n( X, r );
    ...
    c = (SSHORT) (a >> 8);
    a = ( ( (BYTE)a ) >> r );
    printf( "\n==\n c[%04X] a = [%08X] save = [%02X]\n==\n",
           c, a, save );
    X->buf[ X->size - 1 ] = (BYTE) a;
    i = X->size - 2;
    j = P->size - 2;
    if( i < 0 )
    {
