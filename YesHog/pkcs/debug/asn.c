...
RESULT asn_next( ber_blob_p ber )
{
    ...
    if( p + 1 > ber->len )
    {
        ...
        /* debug */
        printf( "buf len[%d] pos[%d], err:(pos + 1 > len)\n",
                ber->len, ber->pos );
        /* end debug */
        goto done;
    }
    ...
    /* debug */
    printf( "Class [%d], Type[%d], Tag[%d]\n",
            ber->tlv->type->asn_class,
            ber->tlv->type->type,
            ber->tlv->type->tag);
    printf( "Len form[%d] len[%d]\n",
            ber->tlv->len->form,
            ber->tlv->len->len );
    /* end debug */
    ...
    /* debug */
    printf( "ber len[%d] pos[%d] payload_len[%d]\n",
            ber->len, ber->pos, ber->payload_len );
    /* end debug */
done:
    return r;
}
...
RESULT asn_check_version( ber_blob_p cert, Cert out )
{
    ...
    /* debug */
    hexdump( cert->cur_pos, out->version_len );
    /* end debug */
...
RESULT asn_check_serialnum( ber_blob_p cert, Cert out )
{
    ...
    /* debug */
    hexdump( cert->cur_pos, out->serialNumber_len );
    /* end debug */
...
RESULT asn_sig_alg( ber_blob_p cert, Cert out, sig_alg_p sa )
{
    ...
    */
    ...
    /* debug */
    hexdump( cert->cur_pos, cert->payload_len );
    /* end debug */
    /* advance to next */
...
RESULT asn_check_issuer( ber_blob_p cert, Cert out )
{
    ...
    /* debug */
    hexdump( cert->cur_pos, out->issuer_len );
    /* end debug */
...
RESULT asn_check_validity( ber_blob_p cert, Cert out )
{
    ...
    /* debug */
    hexdump( cert->cur_pos, out->validityNotBefore_len );
    /* end debug */
    ...
    /* debug */
    hexdump( cert->cur_pos, out->validNotAfter_len );
    /* end debug */
    asn_next_tlv( cert,  VALIDFRM );
...
RESULT asn_check_subject( ber_blob_p cert, Cert out )
{
    ...
    /* debug */
    hexdump( cert->cur_pos, out->subject_len );
    /* end debug */
...
RESULT asn_check_ext(ber_blob_p cert, Cert out)
{
    ...
    /* debug */
    printf( "POS[%d] *POS [%X] LEN [%d]", cert->pos,
            *cert->cur_pos, cert->payload_len );
    printf( "CLASS[%d] TYPE [%d] TAG [%d]\r\n", cert->tlv->type->asn_class,
            cert->tlv->type->type, cert->tlv->type->tag );
    /* end debug */
...
RESULT asn_check_verify_rsa_sha1_sig( BYTE* checksum, Integer C )
{
    ...
    /* debug */
    printf( "ber blob [%X]\n", C->buf[j]);
    hexdump( pvk->cur_pos, pvk->payload_len );
    printf( "\n");
    /* end debug */
    asn_get_next( pvk );
    asn_get_next( pvk );
...
RESULT asn_check_ecdsa_sig_decrypt( Cert out, BYTE* checksum )
{
    ...
    /* debug */
    printf( "r = \n" );
    hexdump( dss->cur_pos, dss->payload_len );
    /* end debug */
    ds.r = dss->cur_pos;
...
RESULT asn_check_verify_sig(Cert out)
{
    ...
    switch( out->caSigAlg.indx )
    {
        ...
        case ALG_ECDSA_SHA1:
            ...
            /* debug */
            hexdump( checksum, sizeof(checksum) );
            /* end debug */
